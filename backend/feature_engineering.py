...
def profile_for_feature_engineering(
    df: pd.DataFrame,
        ...
        for i, col_a in enumerate(corr.columns):
            for col_b in corr.columns[i + 1:]:
                ...
                if pd.notna(value) and value >= 0.9:
                    if target_column and (col_a == target_column or col_b == target_column):
                        continue
...
def apply_feature_engineering_plan(
    ...
    for action in actions:
        if target_column and action.get("column") == target_column:
            continue
