...
DAISY creates its deterministic train/test split from the original upload before fitting imputation, outlier bounds, scaling, and categorical encodings. Only the training fold learns those values; the test fold is transformed with the frozen training values. The package records the random seed, dataset fingerprint, split hashes, feature schema, dependency versions, and exact fitted preprocessing so the experiment can be reproduced and audited. Authentication, per-user artifact authorization, storage quotas, and retention are separate SaaS work.
